...
FIXED_PATTERNS = {
    "01010101": {"follow": "reverse", "label": "8 位交替"},
    "10101010": {"follow": "reverse", "label": "8 位交替"},
    "11111111": {"follow": "1", "label": "8 连长龙"},
    "00000000": {"follow": "0", "label": "8 连长龙"},
}
...
def _build_help_card() -> str:
    return (
        ...
        "• <code>/edb [on/off]</code> 额外长龙下注开关（长龙/交替时是否额外加注 100 万）\n\n"
        "<b>📊 数据管理</b>\n"
...
def _get_dragon_extra_bet_amount(rt: dict, history: list = None) -> int:
    ...
    # 检查交替（8 位纯交替）
    if len(history) >= 8:
        last_8 = ''.join(str(x) for x in history[-8:])
        if last_8 in ('01010101', '10101010'):
            rt["dragon_has_bet"] = True
            rt["dragon_tail_streak"] = 8
            log_event(logging.INFO, 'bet_on', '交替额外加注', user_id=0,
                      data=f"seq={last_8}, history={''.join(str(x) for x in history[-12:])}")
            return 1000000
            ...
            if len(my) > 2 and my[1] == "dl":
                ...
                if len(presets) > 0:
                    ...
                    for name, params in sorted(presets.items()):
                        ...
                        extra_text = " (含额外 100 万)" if dragon_enabled else ""
                        line = f"<code>{name:6}</code> → 基础：{_format_money_message(base_amount):>10} | 最高：{_format_money_message(max_amount):>12}{extra_text}"
                        lines.append(line)
                    
                    preset_table = "\n".join(lines)
                    extra_desc = "• 最高金额：触发长龙 8 连或交替 8 位时的总下注（基础 +100 万）" if rt.get("extra_dragon_bet_enabled", True) else "• 最高金额：与基础金额相同（额外加注已关闭）"
                    mes = (
                        "<b>预设下注金额一览</b>\n\n"
                        f"{preset_table}\n\n"
                        "<b>说明：</b>\n"
