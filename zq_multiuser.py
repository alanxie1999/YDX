...
def _build_help_card() -> str:
    return (
        ...
        "<b>开关控制</b>\n"
        "• <code>/mfb [on/off]</code> 模型兜底开关（模型不可用时是否统计兜底下注）\n"
        "• <code>/edb [on/off]</code> 额外长龙下注开关（长龙/交替时是否额外加注 300 万）\n\n"
        "<b>📊 数据管理</b>\n"
        "• <code>/res tj</code> 重置统计数据（收益、胜率、计数）\n"
        "• <code>/res state</code> 重置状态（包括大路历史）\n"
        "• <code>/res bet</code> 重置押注策略（连押链路清零）\n\n"
        "<b>📋 预设管理</b>\n"
        "• <code>/ys 替换 [名] [参数...]</code> 创建或替换预设\n"
        "• <code>/ys 删除 [名]</code> 删除预设\n"
        "• <code>/ysz [预设名]</code> 查看指定预设的完整倍投序列\n"
        "• <code>/ysz</code> 查看所有预设的倍投序列\n"
        "• <code>/yss</code> 查看预设列表\n"
        "• <code>/yc [预设名]</code> 测算预设倍投序列\n\n"
        "<b>示例：</b>\n"
        "<code>/ys 替换 1w 1 11 3.0 2.5 2.2 2.1 10000</code>\n"
        "<code>/ys 删除 1w</code>"
        "<b>🛠 系统</b>\n"
        "• <code>/ver</code> 查看版本\n"
...
async def _process_bet_on_slim(client, event, user_ctx: UserContext, global_config: dict):
    ...
    # 优先级 0.5: 固定数据规律检测（长龙/交替优先于模式，edb on 时生效）
    elif True:
        fixed_active = False
        if rt.get("extra_dragon_bet_enabled", True):
        ...
        # edb 关闭时跳过固定规律，直接按预设方向或跟随上一手
        if not fixed_active:
            if rt.get("bet_direction") in ("0", "1", "same", "reverse") and history:
...
def _get_dragon_extra_bet_amount(rt: dict, history: list = None) -> int:
    ...
    触发条件：
    - 检测到形态即加注 300 万（无论 lose_count 是多少）
    ...
    # 检查交替（9 位纯交替）
    if len(history) >= 9:
        last_9 = ''.join(str(x) for x in history[-9:])
        if last_9 in ('010101010', '101010101'):
            rt["dragon_has_bet"] = True  # 标记当前下注包含额外加注
            rt["dragon_tail_streak"] = 9
            log_event(logging.INFO, 'bet_on', '交替额外加注', user_id=0,
                      data=f"seq={last_9}, history={''.join(str(x) for x in history[-12:])}")
            return 3000000
    ...
    if streak >= 9:
        ...
        log_event(logging.INFO, 'bet_on', '长龙额外加注', user_id=0,
                  data=f"streak={streak}, side={tail_side}, history={''.join(str(x) for x in history[-12:])}")
        return 3000000
    ...
    try:
        ...
        if cmd == "st" and len(my) > 1:
            ...
            if preset_name in presets:
                ...
                extra_line = "• 额外加注：触发长龙或交替形态时 +300 万\n" if rt.get("extra_dragon_bet_enabled", True) else "• 额外加注：已关闭\n"
                ...
                mes = (
                    ...
                    + extra_line +
                    f"\n执行 <code>/ysz</code> 查看所有预设的完整倍投序列"
                )
                await send_to_admin(client, mes, user_ctx, global_config)
        ...
        if cmd == "mt":
            ...
            extra_line = "• 额外加注：触发长龙或交替形态时 +300 万" if rt.get("extra_dragon_bet_enabled", True) else "• 额外加注：已关闭"
        ...
        if cmd == "edb":
            current_enabled = bool(rt.get("extra_dragon_bet_enabled", True))
            if len(my) == 1:
                state_text = "开启" if current_enabled else "关闭"
                action_text = (
                    "长龙 9 连或交替 9 位时额外加注 300 万"
                    if current_enabled
                    else "长龙/交替形态不触发额外加注"
                )
                mes = _build_ops_card(
                ...
                enabled = my[1].lower() == "on"
                try:
                    rt["extra_dragon_bet_enabled"] = enabled
                    user_ctx.save_state()
                    mes = _build_ops_card(
                    ...
                    mes = _build_ops_card(
                        "额外长龙下注开关设置失败",
                        summary=f"配置写入失败：{str(e)[:80]}",
                        action="正确用法：`edb [on/off]`，例如 `edb off`。",
                    )
            else:
                mes = _build_ops_card(
                    "额外长龙下注开关设置失败",
            ...
            if len(my) > 1:
                ...
                if target_preset not in presets:
                    ...
                    for i in range(1, lose_stop + 1):
                        ...
                        hand_lines.append(f"├ 第{i:2d}手：{_format_money_message(current):>10}（累计：{_format_money_message(total_needed):>10}）{label}")
                ...
                for name in sorted(presets.keys()):
                    ...
                    preset_lines.append(f"├ 【{name}】 {hand_amounts[0]} → {hand_amounts[1]} → {hand_amounts[2]} → {hand_amounts[3]} → {hand_amounts[4]}（总需：{_format_money_message(total)}）")
                ...
                mes = (
                    ...
                    + "\n".join(preset_lines) +
                    f"\n\n⚙️ 押注倍率\n"
                    f"倍投系数：{multipliers[0]} / {multipliers[1]} / {multipliers[2]} / {multipliers[3]}\n\n"
            ...
            if len(my) > 2 and my[1] == "dl":
                ...
                if len(presets) > 0:
                    ...
                    for name, params in sorted(presets.items()):
                        ...
                        dragon_enabled = rt.get("extra_dragon_bet_enabled", True)
                        extra_amount = 1000000 if dragon_enabled else 0
                        max_amount = base_amount + extra_amount
                        extra_text = " (含额外 300 万)" if dragon_enabled else ""
                        line = f"<code>{name:6}</code> → 基础：{_format_money_message(base_amount):>10} | 最高：{_format_money_message(max_amount):>12}{extra_text}"
                        lines.append(line)
                    
                    preset_table = "\n".join(lines)
                    extra_desc = "• 最高金额：触发长龙 5 连或交替 6 位时的总下注（基础 +300 万）" if rt.get("extra_dragon_bet_enabled", True) else "• 最高金额：与基础金额相同（额外加注已关闭）"
                    mes = (
                        "<b>预设下注金额一览</b>\n\n"
                        f"{preset_table}\n\n"
                        "<b>说明：</b>\n"
        ...
        if cmd == "ys":
            if len(my) < 3:
                mes = (
                    "📝 **预设创建/修改/删除命令**\n\n"
                    "用法：\n"
                    "`ys 替换 [预设名] [continuous] [lose_stop] [lose_once] [lose_twice] [lose_three] [lose_four] [initial_amount]`\n"
                    "`ys 删除 [预设名]`\n\n"
                    "参数说明：\n"
                    ...
                    "例：\n"
                    "`ys 替换 1w 1 11 3.0 2.5 2.2 2.1 10000`\n"
                    "`ys 删除 1w`"
                )
                message = await send_to_admin(client, mes, user_ctx, global_config)
                asyncio.create_task(delete_later(client, event.chat_id, event.id, 10))
            ...
            if action in ["删除", "del", "delete", "rm"]:
                ...
                if preset_name not in presets:
                    mes = f"❌ 预设不存在：{preset_name}\n\n执行 `/yss` 查看可用预设"
                else:
                    del presets[preset_name]
                    user_ctx.save_presets()
                    mes = f"✅ 预设已删除：{preset_name}"
                    log_event(logging.INFO, 'user_cmd', '删除预设', user_id=user_ctx.user_id, preset=preset_name)
            ...
            # 替换/创建预设
            if action in ["替换", "replace", "set", "update"]:
                preset_name = my[2]
                try:
                    # 解析参数（支持部分参数，使用默认值）
                    continuous = int(my[3]) if len(my) > 3 else 1
                    lose_stop = int(my[4]) if len(my) > 4 else 8
                    lose_once = float(my[5]) if len(my) > 5 else 3.0
                    lose_twice = float(my[6]) if len(my) > 6 else 2.5
                    lose_three = float(my[7]) if len(my) > 7 else 2.2
                    lose_four = float(my[8]) if len(my) > 8 else 2.1
                    initial_amount = int(my[9]) if len(my) > 9 else 5000
                    bet_direction = my[10] if len(my) > 10 else "auto"
                    auto_pause_count = my[11] if len(my) > 11 else "0"
                    ...
                    mes = (
                        f"✅ 预设{'替换' if is_update else '创建'}成功：{preset_name}\n\n"
                        f"参数：\n"
                        f"• 连续下注：{continuous}\n"
                        f"• 最大连投：{lose_stop} 手\n"
                    ...
                    log_event(logging.INFO, 'user_cmd', '替换/创建预设', user_id=user_ctx.user_id, preset=preset_name, is_update=is_update)
                    message = await send_to_admin(client, mes, user_ctx, global_config)
                    asyncio.create_task(delete_later(client, event.chat_id, event.id, 10))
                    if message:
                        asyncio.create_task(delete_later(client, message.chat_id, message.id, 60))
                    ...
                    mes = (
                        f"❌ 预设{'替换' if preset_name in presets else '创建'}失败\n\n"
                        f"错误：参数格式不正确\n\n"
                        f"请执行 `ys` 查看命令用法"
                    )
                    log_event(logging.WARNING, 'user_cmd', '替换/创建预设失败', user_id=user_ctx.user_id, preset=preset_name, error=str(e))
                    message = await send_to_admin(client, mes, user_ctx, global_config)
                    asyncio.create_task(delete_later(client, event.chat_id, event.id, 10))
                    if message:
                        asyncio.create_task(delete_later(client, message.chat_id, message.id, 10))
