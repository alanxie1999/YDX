...
更新内容：
- 长龙额外加注触发改回第 6 手：历史尾部连续同向或连续交替达到 6 连（`streak >= 6`）时本手下注额外加 100 万。
- 同向长龙额外押同向，交替长龙额外押交替，任意下注方向模式均触发，赢了继续、不中后停止。
...
更新内容：
- 长龙额外加注触发改为第 7 手：历史尾部连续同向或连续交替达到 7 连（`streak >= 7`）时本手下注额外加 100 万。
- 同向长龙额外押同向，交替长龙额外押交替，任意下注方向模式均触发。
- 额外加注赢了继续、不中后停止。
...
更新内容：
- 取消长龙额外加注的模式门控：同向长龙和交替长龙在任意下注方向模式（MT/ST/auto/固定方向）下均触发。
- 同向长龙额外押同向，交替长龙额外押交替，方向始终跟随长龙形态。
...
- 长龙额外加注同时支持同向长龙和交替长龙。
- 同向长龙额外押同向，交替长龙额外押交替。
- MT（反向下注）模式下仅同向长龙触发额外加注，且额外押同向。
- ST（同向下注）模式下仅交替长龙触发额外加注，且额外押交替。
- 固定方向预设（0/1）加注金额照常触发，方向保持预设设定。
- 额外加注赢了继续、错了立即停止。
- 连错后仍可在长龙盘面触发额外加注。

重点变化：
- 6 连同向或 6 连交替都会额外加 100 万。
- MT 模式遇同向长龙额外押同向，ST 模式遇交替长龙额外押交替。
- 加注手中了后下一手继续加注，不中后立刻回到默认金额。
- 马丁连错 6 手后，第 7 手遇到长龙仍会触发额外加注。
...
更新内容：
- 迁移 v2.0.3 的 `mt` 命令：`mt [预设名]` 加载预设并开启交替模式（`bet_direction` 强制为反向：开 1 押 0，开 0 押 1），支持不带参数沿用当前预设，未指定预设时给出可用预设列表，帮助文档同步补充说明。
...
更新内容：
- 移除固定金额模式（`fix*` 预设）连输达到上限后自动暂停 10 局的功能：结算不再标记自动暂停，相关通知、`status`/`yss` 展示、`ys` 保存参数、内置预设参数与帮助文案一并清理，固定金额预设仅保留方向设定，连输后由统一的连投上限止损机制接管。
...
更新内容：
- `yss [预设名]` / `ysz [预设名]` 支持查看指定预设详情：显示最多连投手数、逐手倍投金额与累计资金、总需资金、倍率、押注方向、自动暂停和长龙加注状态（兼容 v2.0.3 的 `ysz [预设名]` 行为）。
...
更新内容：
- 恢复 `ysz` 命令作为 `yss`（查看/删除预设）的兼容别名，修复旧版本 `ysz` 变为未知命令的问题。
- `ys` 命令兼容 v2.0.3 的 `ys 替换 [预设名] ...` 与 `ys 删除 [预设名]` 格式，同时支持可选的第 9/10 个参数（押注方向、自动暂停），新旧格式均可正常保存预设。
...
更新内容：
- 长龙额外加注触发改为第 6 手：历史尾部连续同向达到 6 连（`streak >= 6`）时本手下注额外加 100 万，触发后每手继续加注直到不中停止。
...
- `edb off` 模式下现在只按预设方向下注：强制延续下注（`forced_bet_remaining`）、固定规律检测、交替打破、长龙加注等动态策略全部禁用；方向仅由预设 `bet_direction` 决定（未设方向时按 `auto` 跟随上一手）。
- 关闭 `edb` 时会自动清除之前规律检测遗留的强制延续状态。
...
- 只要最近 **5 位** 不再是纯交替（10101/01010），系统就会自动退出交替增强逻辑，恢复到原来的主策略判断；原有的模型异常保护、统计兜底、连续观望解锁与暂停逻辑保持不变。
