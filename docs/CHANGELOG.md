...
更新内容：
- 新增 `edb` 开关命令，用于控制"长龙/交替形态时是否额外加注 100 万"。
- 当关闭额外长龙下注后，长龙 5 连或交替 6 位不再触发额外加注，走正常倍投链。
- 该开关为运行态配置，与 `mfb` 一样由命令直接控制。
- 新增 `mfb` 开关命令，用于控制“模型链不可用时，是否继续统计兜底下注”。
- 当关闭统计兜底下注后，模型链不可用时本局将不再下注，而是等待模型恢复并继续后台探测。
- 该开关改为账号运行态配置，和 `wlc` 一样由命令直接控制，不再写入账号 `ai` 配置。

重点变化：
- `edb on` 时保持现有行为：长龙 5 连或交替 6 位时额外加注 100 万。
- `edb off` 时改为按标准倍投链下注，不触发额外加注。
